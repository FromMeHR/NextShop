...
function getMask(
  ...
  if (!predefinedMask || alwaysDefaultMask) {
    return prefix + formattedDialCode + " " + defaultMask;
  } else {
    return prefix + formattedDialCode + " " + predefinedMask;
  }
}
      ...
      country[6] &&
        country[6].forEach((areaCode) => {
          ...
          const formattedDialCode = areaItem.dialCode
            ...
            .map((char) => (char === " " ? " " : "."))
            .join("");
          if (!country[4] || alwaysDefaultMask) {
            areaCodeFormat = prefix + formattedDialCode + " " + defaultMask.replace(/^[.]+\s/, "");
          } else {
            areaCodeFormat = prefix + formattedDialCode + " " + country[4].replace(/^[.]+\s/, "");
          }

          areaItem.format = areaCodeFormat;
...
export default class CountryData {
  constructor(
    ...
    if (enableTerritories) {
      let [initializedTerritories] = initCountries(
        rawTerritories,
        enableAreaCodes,
        prefix,
        defaultMask,
  ...
  getFilteredCountryList = (countryCodes, sourceCountryList, preserveOrder) => {
    ...
    if (preserveOrder) {
      ...
      filteredCountries = countryCodes
        .map((countryCode) => {
          return sourceCountryList.find(
            (country) => country.iso2 === countryCode
          );
        })
        .filter((countryCode) => {
          return sourceCountryList.some(
            (country) => country.iso2 === countryCode
          );
        });
    } else {
