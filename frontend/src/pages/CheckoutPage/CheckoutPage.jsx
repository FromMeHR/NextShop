...
export function CheckoutPage() {
  ...
  const [dropdownCityPosition, setDropdownCityPosition] = useState("");
  const selectCityBoxRef = useRef(null);
  ...
  const [isDropdownWarehouseOpen, setIsDropdownWarehouseOpen] = useState(false);
  const [dropdownWarehousePosition, setDropdownWarehousePosition] = useState("");
  const selectWarehouseBoxRef = useRef(null);
  const dropdownWarehouseRef = useRef(null);

  const [streets, setStreets] = useState([]);
  const [searchStreet, setSearchStreet] = useState("");
  const [filteredStreets, setFilteredStreets] = useState([]);
  const [selectedStreet, setSelectedStreet] = useState(null);
  const [isDropdownStreetOpen, setIsDropdownStreetOpen] = useState(false);
  const [dropdownStreetPosition, setDropdownStreetPosition] = useState("");
  const selectStreetBoxRef = useRef(null);
  const dropdownStreetRef = useRef(null);
  ...
  const { error: citiesError, isLoading: citiesLoading } = useSWR(
    debouncedSearch && debouncedSearch.length >= 2
      ? [process.env.REACT_APP_BASE_API_URL, debouncedSearch]
      : null,
    fetchCities,
  ...
  const updateDropdownCityPosition = useCallback(() => {
    if (!selectCityBoxRef.current || !dropdownCityRef.current) return;

    const rect = selectCityBoxRef.current.getBoundingClientRect();
    const dropdownHeight = dropdownCityRef.current.offsetHeight;
    ...
    const isContentReady = !citiesLoading && cities.length > 0;
    const resolvedPosition =
      isContentReady || cities.length === 0
        ? shouldOpenAbove
          ? "above"
          : "below"
        : dropdownCityPosition;
    setDropdownCityPosition(resolvedPosition);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        isDropdownCityOpen &&
        !selectCityBoxRef.current?.contains(event.target) &&
    ...
    if (isDropdownCityOpen) {
      updateDropdownCityPosition();
      window.addEventListener("resize", updateDropdownCityPosition);
      window.addEventListener("scroll", updateDropdownCityPosition);
      document.addEventListener("mousedown", handleClickOutside);
    }

    return () => {
      window.removeEventListener("resize", updateDropdownCityPosition);
      window.removeEventListener("scroll", updateDropdownCityPosition);
      document.removeEventListener("mousedown", handleClickOutside);
    };
  }, [isDropdownCityOpen, updateDropdownCityPosition]);
  ...
  const { error: warehouseTypesError, isLoading: warehouseTypesLoading } =
    useSWR(
      selectedCity?.ref
        ? [process.env.REACT_APP_BASE_API_URL, selectedCity]
        : null,
      fetchWarehouseTypes,
      { 
  ...
  useSWR(selectedDeliveryType === 1 && selectedCity?.ref && selectedWarehouseType?.ref
    ? [process.env.REACT_APP_BASE_API_URL, selectedCity, selectedWarehouseType]
      : null,
    fetchWarehouses,
    {
      onSuccess: (data) => {
  ...
  const updateDropdownWarehousePosition = useCallback(() => {
    if (!selectWarehouseBoxRef.current || !dropdownWarehouseRef.current) return;

    const rect = selectWarehouseBoxRef.current.getBoundingClientRect();
    const dropdownHeight = dropdownWarehouseRef.current.offsetHeight;
    const spaceBelow = window.innerHeight - rect.bottom;
    const spaceAbove = rect.top;
    ...
    setDropdownWarehousePosition(dropdownPosition);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !selectWarehouseBoxRef.current?.contains(event.target) &&
        !dropdownWarehouseRef.current?.contains(event.target)
      ) {
        setIsDropdownWarehouseOpen(false);
      }
    };

    if (isDropdownWarehouseOpen) {
      updateDropdownWarehousePosition();
      window.addEventListener("resize", updateDropdownWarehousePosition);
      window.addEventListener("scroll", updateDropdownWarehousePosition);
      document.addEventListener("mousedown", handleClickOutside);
    }

    return () => {
      window.removeEventListener("resize", updateDropdownWarehousePosition);
      window.removeEventListener("scroll", updateDropdownWarehousePosition);
      document.removeEventListener("mousedown", handleClickOutside);
    };
  }, [isDropdownWarehouseOpen, filteredWarehouses, updateDropdownWarehousePosition]);
  ...
  useSWR(selectedDeliveryType === 2 && selectedCity?.ref
    ? [process.env.REACT_APP_BASE_API_URL, selectedCity.ref]
      : null,
    fetchStreets,
    {
      onSuccess: (data) => {
  ...
  const updateDropdownStreetPosition = useCallback(() => {
    if (!selectStreetBoxRef.current || !dropdownStreetRef.current) return;

    const rect = selectStreetBoxRef.current.getBoundingClientRect();
    const dropdownHeight = dropdownStreetRef.current.offsetHeight;
    const spaceBelow = window.innerHeight - rect.bottom;
    const spaceAbove = rect.top;
    ...
    const dropdownPosition = shouldOpenAbove ? "above" : "below";
    setDropdownStreetPosition(dropdownPosition);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        isDropdownStreetOpen &&
        !selectStreetBoxRef.current?.contains(event.target) &&
        !dropdownStreetRef.current?.contains(event.target)
      ) {
        setIsDropdownStreetOpen(false);
      }
    };

    if (isDropdownStreetOpen) {
      updateDropdownStreetPosition();
      window.addEventListener("resize", updateDropdownStreetPosition);
      window.addEventListener("scroll", updateDropdownStreetPosition);
      document.addEventListener("mousedown", handleClickOutside);
    }

    return () => {
      window.removeEventListener("resize", updateDropdownStreetPosition);
      window.removeEventListener("scroll", updateDropdownStreetPosition);
      document.removeEventListener("mousedown", handleClickOutside);
    };
  }, [isDropdownStreetOpen, filteredStreets, updateDropdownStreetPosition]);
        ...
        <>
          {isLoading ? <Loader /> :
            <div className={css["cart-empty"]}>
              ...
              >
                <img
                  src={`${process.env.REACT_APP_PUBLIC_URL}/img/robot.png`}
                  alt="Robot"
